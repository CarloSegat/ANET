...
public class MovingHonestNode extends Node {
    ...
    boolean isConflict(List<Node> l) {
        for (Node node : l) {
            if (color == ((MovingHonestNode) node).color) {
                return true;
            }
        }
        return false;
    ...
    void findMinimalColor(List<Node> l) {
        List<Integer> colors = l.stream().map(n -> ((MovingHonestNode) n).color).collect(Collectors.toList());
    ...
    void takeColorOfNeighbour(List<Node> l) {
        ...
        setColor(((MovingHonestNode)l.get(randomNeigh)).getColorAsInt());
    }
