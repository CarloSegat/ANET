...
public class MinimalColoringNode extends Node {
    ...
    boolean isConflict(List<Node> l) {
        for (Node node : l) {
            if (color == ((MinimalColoringNode) node).color) {
                return true;
            }
        }
        return false;
    ...
    void findMinimalColor(List<Node> l) {
        List<Integer> colors = l.stream().map(n -> ((MinimalColoringNode) n).color).collect(Collectors.toList());
    ...
    void takeColorOfNeighbour(List<Node> l) {
        ...
        setColor(((MinimalColoringNode)l.get(randomNeigh)).getColorAsInt());
    }
